...
void Texture::setTexture()
{
    glGenTextures(1, &mId);
    glBindTexture(GL_TEXTURE_2D, mId);
    qDebug() << "Texture" << mTextureFilename.c_str() << "successfully read | id = " << mId << "| bytes pr pixel:" << mBytesPrPixel << "| using alpha:" << mAlphaUsed << "| w:" << mColumns << "|h:" << mRows;
...
void Texture::setCubemapTexture()
{
    ...
    if(!mAlphaUsed)  //no alpha in this bmp
        ...
        qDebug() << "Skybox with alpha does not make sense";

    glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
    glBindTexture(GL_TEXTURE_CUBE_MAP, 0);

    qDebug() << "Cubemap Texture" << mTextureFilename.c_str() << "successfully read | id = "
             << mId << "| bytes pr pixel:" << mBytesPrPixel << "| using alpha:" << mAlphaUsed
             << "| w:" << mColumns << "|h:" << mRows;
}
